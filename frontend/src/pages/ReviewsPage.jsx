...
export default function ReviewsPage() {
  ...
  const fetchProposalAndReviews = async () => {
    try {
      ...
      const currentProp = allProps.find(p => String(p.id) === String(proposalId));
      ...
      if (currentProp) {
        ...
        const partnerName = isSender ? (currentProp.receiver_name || 'Partner') : (currentProp.sender_name || 'Partner');
        ...
        try {
          const response = await fetch(`/api/reviews/received/?user_id=${partnerId}`, {
            method: 'GET',
            headers: {
  ...
  const getRatingCount = (star) => targetUserReviews.filter(r => Number(r.rating) === star).length;
  ...
  const averageRating = (targetUserReviews.reduce((acc, r) => acc + Number(r.rating), 0) / (targetUserReviews.length || 1)).toFixed(1);

  const sortedReviews = [...targetUserReviews].sort((a, b) => new Date(b.created_at || 0) - new Date(a.created_at || 0));

  return (
    <div className="max-w-6xl mx-auto px-4 py-12 space-y-10">
      <div className="text-center space-y-3">
        <h1 className="text-3xl md:text-4xl font-bold tracking-tight text-blue-600">
          Leave a Review <span className="text-slate-800 text-2xl font-semibold"></span>
        </h1>
      ...
      <div className="bg-white p-6 md:p-8 rounded-2xl border border-slate-200/80 shadow-sm grid grid-cols-1 md:grid-cols-3 gap-6 items-center">
        <div className="text-center md:text-left border-b md:border-b-0 md:border-r border-slate-100 pb-6 md:pb-0 md:pr-6">
          <h3 className="text-slate-500 font-bold text-sm uppercase tracking-wider">{targetUserName}'s Rating</h3>
          <div className="text-4xl md:text-5xl font-black text-slate-900 mt-2">
            {targetUserReviews.length > 0 ? averageRating : '0.0'}
          </div>
          <p className="text-xs font-bold text-slate-400 mt-1">{targetUserReviews.length} total reviews</p>
        </div>
      ...
      {proposalId && (
        <div className="bg-white p-6 md:p-8 rounded-2xl border border-slate-200/80 shadow-sm space-y-6">
          <h2 className="text-xl font-bold text-slate-900">Write a Review for {targetUserName}</h2>
          ...
          <form onSubmit={handleReviewSubmit} className="space-y-4">
            ...
            <div>
              <label className="block text-xs font-bold text-slate-600 uppercase mb-1">Comment</label>
              <textarea
                rows="4"
                value={comment}
      ...
      <div className="space-y-6">
        ...
        {sortedReviews.length === 0 ? (
          <div className="bg-white border border-slate-200/80 rounded-2xl p-10 text-center text-slate-400 shadow-sm">
            ...
            <p className="mt-2 text-sm font-bold">No reviews found for {targetUserName} yet.</p>
          </div>
        ) : (
